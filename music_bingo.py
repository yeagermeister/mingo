...
class DisplayWindow:
    ...
    def update_songs(self, songs):
        ...
        # Display last 10 songs (most recent first)
        display_songs = songs[-10:] if len(songs) > 10 else songs
        for i, song in enumerate(reversed(display_songs)):
            if i < 10:
                text = f"{len(songs) - i}. {song['title']} - {song['artist']}"
                self.song_labels[i].config(text=text, foreground='black')
